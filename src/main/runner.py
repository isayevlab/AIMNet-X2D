...
def main_runner(args) -> dict[str, Any]:
    ...
    if handle_inference_mode(args):
        ...
        if is_ddp and dist.is_initialized():
            logger.info(f"Rank {local_rank}: Cleaning up distributed resources...")
            try:
                # Final barrier to ensure all ranks finish
                dist.barrier()  # FIXED: Removed timeout
                logger.info(f"Rank {local_rank}: Final barrier passed")
            except Exception as e:
                logger.warning(f"Rank {local_rank}: Barrier error (may be expected): {e}")
            ...
            try:
                ...
                logger.error(f"Rank {local_rank}: Error destroying process group: {e}")
    ...
    try:
        ...
        if is_ddp and dist.is_initialized():
            logger.info(f"Rank {local_rank}: Final cleanup...")
            try:
            ...
            try:
                dist.destroy_process_group()
                logger.info(f"Rank {local_rank}: Cleanup complete")
            except RuntimeError:
...
def _run_inference_mode(args, device, is_ddp, local_rank, world_size) -> dict[str, Any]:
    ...
    if args.inference_hdf5:
        try:
            ...
            if is_main_process() and total_molecules:
                logger.info(f"Processing {total_molecules:,} molecules from HDF5")
    ...
    if is_ddp and dist.is_initialized():
        logger.info(f"Rank {local_rank}: Waiting for all ranks to finish...")
        try:
            dist.barrier()  # FIXED: Removed timeout parameter
            logger.info(f"Rank {local_rank}: All ranks completed")
        except Exception as e:
            logger.error(f"Rank {local_rank}: Barrier error: {e}")
...
def _run_training_mode(args, device, is_ddp, local_rank, world_size) -> dict[str, Any]:
    ...
    if is_main_process():
        ...
        logger.info("=" * 80)
        logger.info(f"Rank 0: Starting model save to: {args.model_save_path}")
        ...
        try:
            ...
            logger.info(f"Rank 0: Model saved successfully:")
            logger.info(f"   Path: {args.model_save_path}")
            logger.info(f"   Size: {file_size:,} bytes ({file_size/1024/1024:.2f} MB)")

        except Exception as e:
            logger.error(f"Rank 0: FAILED to save model: {e}")
            raise

    # CRITICAL FIX: All ranks must wait for rank 0 to finish saving
    if is_ddp and dist.is_initialized():
        if not is_main_process():
            logger.info(f"Rank {local_rank}: Waiting for rank 0 to finish saving model...")

        dist.barrier()

        if is_main_process():
            logger.info(f"Rank 0: All ranks synchronized after model save")
        else:
            logger.info(f"Rank {local_rank}: Rank 0 finished saving, continuing...")
...
def _load_and_preprocess_data(args) -> dict[str, Any]:
    ...
    if args.iterable_dataset:
        ...
        if _check_hdf5_files_exist(args):
            ...
            if is_main_process():
                logger.info("HDF5 files not found - performing full preprocessing...")
    ...
    if is_main_process():
        logger.info(f"Data loaded: {len(smiles_train)} train, {len(smiles_val)} val, {len(smiles_test)} test")
        logger.info(f"Number of tasks: {data_info['num_tasks']}")
...
def _preprocess_for_hdf5_storage(args, smiles_train, target_train, smiles_val, target_val, smiles_test, target_test) -> dict[str, Any]:
    ...
    """
    if is_main_process():
        logger.info("HDF5 mode: Applying FULL preprocessing (SAE -> Standard Scaling) and storing in HDF5...")
    ...
    if is_main_process():
        logger.info("Full preprocessing applied for HDF5 storage:")
        if args.calculate_sae:
            logger.info("   1. SAE normalization: applied")
        if preprocessing_config.apply_standard_scaling:
            logger.info("   2. Standard scaling: applied")
        logger.info("   -> Processed data will be stored in HDF5")
...
def _preprocess_for_inmemory_storage(args, smiles_train, target_train, smiles_val, target_val, smiles_test, target_test) -> dict[str, Any]:
    ...
    """
    if is_main_process():
        logger.info("In-memory mode: Applying preprocessing for immediate use...")
    ...
    if is_main_process():
        logger.info("Preprocessing applied for in-memory datasets")
...
def _create_in_memory_datasets(args, data_info: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_main_process():
        logger.info("Creating in-memory datasets...")
...
def _create_hdf5_datasets(args, data_info: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_main_process():
        logger.info("Setting up HDF5 datasets...")
    ...
    if need_precompute:
        if is_main_process():
            logger.info("Creating HDF5 files with PREPROCESSED data...")
            logger.info("   -> SAE normalization: applying during HDF5 creation")
            logger.info("   -> Standard scaling: applying during HDF5 creation")
            logger.info("   -> HDF5 will contain ready-to-use processed targets")
            logger.info("   -> Storing preprocessing statistics in HDF5 metadata")
        ...
        if is_main_process():
            ...
            logger.info("HDF5 files created with FULLY PREPROCESSED data and metadata")
    else:
        if is_main_process():
            logger.info("Using existing HDF5 files with PREPROCESSED data")
        ...
        with h5py.File(hdf5_path, 'a') as f:  # Open in append mode
            if 'metadata' in f:
                ...
                if preprocessing_pipeline.standard_scaler and preprocessing_pipeline.standard_scaler.is_fitted:
                    ...
                    logger.info(f"   -> Stored scaler statistics in {split_name} HDF5: means={preprocessing_pipeline.standard_scaler.means}, stds={preprocessing_pipeline.standard_scaler.stds}")
                ...
                if preprocessing_pipeline.sae_normalizer and preprocessing_pipeline.sae_normalizer.is_fitted:
                    if preprocessing_pipeline.sae_normalizer.sae_statistics:
                        ...
                        logger.info(f"   -> Stored SAE statistics in {split_name} HDF5")
...
def _load_pretrained_weights(model: torch.nn.Module, args) -> None:
    ...
    if is_main_process():
        logger.info(f"Loading pretrained weights from {args.transfer_learning}")
    ...
    if output_layer_key in state_dict and output_layer_key in model_state:
        ...
        if pretrained_shape != current_shape:
            if is_main_process():
                logger.info(f"Output dimension mismatch detected:")
                logger.info(f"  Pretrained: {pretrained_shape}")
                logger.info(f"  Current: {current_shape}")
                logger.info(f"  Excluding output layer from transfer learning")
    ...
    if args.freeze_pretrained:
        if is_main_process():
            logger.info("Freezing pretrained layers (output layer remains trainable)")
...
def _extract_embeddings_if_requested(args, model: torch.nn.Module, data_loaders: dict[str, Any],
    ...
    logger.info(f"Embeddings saved to: {args.embeddings_output_path}")
...
def _save_best_model(args, model: torch.nn.Module, preprocessing_pipeline,
    ...
    try:
        logger.info(f"Rank 0: Writing {len(model_state_dict)} parameter tensors to disk...")
        torch.save(model_artifact, args.model_save_path)

        # CRITICAL FIX: Force OS to flush buffers to disk
        logger.info(f"Rank 0: Syncing file buffers to disk...")
        with open(args.model_save_path, 'rb') as f:
            os.fsync(f.fileno())

        # Verify the save
        actual_size = os.path.getsize(args.model_save_path)
        logger.info(f"Model artifact written and synced to disk")
        logger.info(f"   File: {args.model_save_path}")
        logger.info(f"   Size: {actual_size:,} bytes ({actual_size/1024/1024:.2f} MB)")

    except Exception as e:
        logger.error(f"CRITICAL ERROR during model save: {e}")
        logger.error(f"   Attempted path: {args.model_save_path}")
        raise
...
def _check_hdf5_files_exist(args) -> bool:
    ...
    if files_exist and is_main_process():
        logger.info("All HDF5 files found:")
        logger.info(f"   - Train: {train_hdf5_path}")
        logger.info(f"   - Val: {val_hdf5_path}")
        logger.info(f"   - Test: {test_hdf5_path}")
...
def _load_hdf5_preprocessing_info(args, smiles_train, target_train, smiles_val,
    ...
    if is_main_process():
        logger.info("=" * 80)
        logger.info("LOADING EXISTING HDF5 FILES")
        logger.info("=" * 80)
    ...
    if is_main_process():
        logger.info(f"Successfully loaded preprocessing pipeline from HDF5")
        logger.info(f"  - SAE applied: {sae_applied}")
        logger.info(f"  - Scaler means: {actual_scaler_means}")
        logger.info(f"  - Scaler stds: {actual_scaler_stds}")
...
def run_single_trial(args) -> dict[str, Any]:
    ...
    if not hasattr(args, '_trial_temp_dir') and not hasattr(args, '_is_hyperopt_trial'):
        ...
        if is_main_process():
            logger.info(f"Model ready for evaluation (will save best after all trials)")
