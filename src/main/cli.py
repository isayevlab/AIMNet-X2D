...
def parse_main_arguments(args: list | None = None) -> argparse.Namespace:
    """
...
def print_configuration(args: argparse.Namespace) -> None:
    ...
    logger.info("=" * 80)
    logger.info("AIMNET-X2D CONFIGURATION")
    logger.info("=" * 80)

    # Data configuration
    logger.info("Data Configuration:")
    if args.data_path:
        logger.info(f"  Single file: {args.data_path}")
        logger.info(f"  Train/Val/Test splits: {args.train_split}/{args.val_split}/{args.test_split}")
    else:
    ...
    if args.task_type == "multitask":
        logger.info(f"  Target columns: {args.multi_target_columns}")
    else:
    ...
    logger.info(f"  Number of shells: {args.num_shells}")
    logger.info(f"  Message passing layers: {args.num_message_passing_layers}")
    logger.info(f"  Pooling type: {args.pooling_type}")
    logger.info(f"  Activation: {args.activation_type}")
    logger.info(f"  Partial charges: {args.use_partial_charges}")
    logger.info(f"  Stereochemistry: {args.use_stereochemistry}")
    logger.info("")
    ...
    if args.loss_function == "evidential":
        logger.info(f"  Evidential lambda: {args.evidential_lambda}")
    ...
    if args.hyperparameter_file and args.num_trials > 1:
        ...
        logger.info(f"  Configuration file: {args.hyperparameter_file}")
        logger.info(f"  Number of trials: {args.num_trials}")
        logger.info("")

    # System configuration
    logger.info("System Configuration:")
    logger.info(f"  GPU devices: {args.num_gpu_devices}")
    logger.info(f"  Workers: {args.num_workers}")
    logger.info(f"  Mixed precision: {args.mixed_precision}")
    logger.info(f"  Iterable dataset: {args.iterable_dataset}")
