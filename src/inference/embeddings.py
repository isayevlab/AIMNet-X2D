...
class EmbeddingExtractor:
    ...
    def __init__(self, model: nn.Module, device: torch.device) -> None:
        self.model = model
        self.device = device
        self.mol_embeddings: list[Any] = []
        self.atom_embeddings: list[Any] = []
        self.batch_indices: list[Any] = []
        self.smiles_list: list[str] = []
        self.hooks: list[Any] = []

    def setup_hooks(self, include_atom_embeddings: bool = False) -> None:
            ...
            if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
                ...
                if hasattr(self.model.module, 'concat_self_other'):
                    ...
                    if self.model.module.message_passing_layers:
                        ...
                        logger.warning("Could not find suitable layer for atom embeddings")
                        return
            else:
    ...
    def extract_batch_embeddings(self, batch) -> dict[str, Any]:
        """
...
class StreamingEmbeddingWriter:
    ...
    def __init__(self,
        ...
        self.all_mol_embeddings: list[Any] = []
        self.all_smiles: list[str] = []
        self.all_atom_embeddings: dict[int, Any] = {}
        self.all_atom_counts: list[int] = []
    ...
    def write_batch(self, embeddings_data: dict[str, Any]) -> None:
        """
        ...
        if self.rank == 0 and self.current_index % DEFAULT_PROGRESS_LOG_INTERVAL == 0:
            if self.include_atom_embeddings:
                logger.info("Processed %d molecules (mol + atom embeddings)", self.current_index)
            else:
                logger.info("Processed %d molecules (mol embeddings)", self.current_index)

        # Periodic flush to avoid memory issues
        if len(self.all_mol_embeddings) > DEFAULT_BATCH_FLUSH_THRESHOLD:
            self._partial_write()

    def _partial_write(self) -> None:
    ...
    def finalize(self) -> None:
        ...
        if self.file is not None:
            ...
            if is_main_process():
                logger.info("Rank %d wrote %d embeddings to %s", self.rank, len(self.all_smiles), self.rank_output_path)
                if self.include_atom_embeddings:
                    logger.info("Rank %d wrote %d atom embedding groups", self.rank, len(self.all_atom_embeddings))

    def combine_ddp_files(self) -> None:
        ...
        logger.info("Combining embeddings from %d ranks...", self.world_size)

        all_embeddings: list[Any] = []
        all_smiles: list[str] = []
        all_atom_embeddings: dict[int, Any] = {}
        all_atom_counts: list[int] = []
        ...
        for rank in range(self.world_size):
            ...
            if rank_file.exists():
                try:
                    ...
                    logger.error("Error reading rank %d file: %s", rank, e)
        ...
        if all_embeddings and all_smiles:
            ...
            logger.info("Combined %d embeddings from %d ranks", len(all_smiles), self.world_size)
            if self.include_atom_embeddings:
                logger.info("Combined %d atom embedding groups", len(all_atom_embeddings))
            logger.info("Final embeddings saved to: %s", self.output_path)
